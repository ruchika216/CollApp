  ...
  return (
    ...
    >
      ...
      <View style={styles.metaRow}>
        ...
        <View style={styles.metaItem}>
          <Icon name="link" size={14} tintColor={textColor} />
          <Text style={[styles.metaText, { color: textColor }]}>
            {' '}
        ...
        <View style={styles.metaItem}>
          <Icon name="dashboard" size={14} tintColor={textColor} />
          <Text style={[styles.metaText, { color: textColor }]}>
            {' '}
            {task.viewCount ?? 0}
          </Text>
        ...
        <View style={styles.metaItem}>
          <Icon name="message" size={14} tintColor={textColor} />
          <Text style={[styles.metaText, { color: textColor }]}>
            {' '}
        ...
        <View style={styles.avatars}>
          {(Array.isArray(task.assignedTo)
            ...
            >
              <Image
                source={{
                  uri: `https://api.dicebear.com/7.x/initials/png?seed=${id}`,
                }}
                style={styles.avatar}
              />
            </View>
          ...
          {Array.isArray(task.assignedTo) && task.assignedTo.length > 3 && (
            <View style={[styles.moreAvatar, { left: 3 * -12 }]}>
...
const styles = StyleSheet.create({
  card: {
    ...
    borderWidth: 1.25,
    marginBottom: 0,
    // subtle elevation so cards feel a bit "above" the background
    shadowColor: '#000',
    shadowOpacity: 0.08,
    shadowRadius: 6,
  ...
  moreAvatarText: { fontSize: 12, fontWeight: '800' },
  actionPlus: {
    position: 'absolute',
    right: 12,
    bottom: 12,
