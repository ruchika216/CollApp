...
import { fetchTasks, fetchUserTasks, createTask, updateTask, deleteTask, subscribeToTasks, unsubscribeFromTasks } from '../store/slices/taskSlice';
...
const TaskScreen: React.FC<TaskScreenProps> = ({ navigation }) => {
  ...
  const [expandedComments, setExpandedComments] = useState<{[taskId: string]: boolean}>({});
  ...
  useEffect(() => {
    const initializeData = async () => {
      ...
      dispatch(fetchApprovedUsers()).then((result) => {
  ...
  const handleSubmit = async () => {
    ...
    if (!selectedTask && (!taskForm.title.trim() || !taskForm.description.trim() || !taskForm.assignedUser.trim())) {
      ...
      if (selectedTask) {
        ...
        if (isAdmin) {
          ...
          if (taskForm.comment.trim()) {
            ...
            updateData.comments = selectedTask.comments ? [...selectedTask.comments, newComment] : [newComment];
        ...
        Alert.alert('Success', isAdmin ? 'Task updated successfully' : 'Task status updated successfully');
      } else {
  ...
  const getAssigneeName = (userId: string) => {
    ...
    return assignedUser?.name || assignedUser?.email?.split('@')[0] || 'Unknown User';
  };
  ...
  const EmptyTasksComponent = () => (
    <View style={styles.emptyState}>
      ...
      <Text style={[styles.emptySubtitle, { color: colors.textSecondary }]}>
        {isAdmin ? 'Create a new task to get started' : 'No tasks assigned to you for this date'}
      </Text>
    </View>
  );
  ...
  const renderTaskItem = ({ item }: { item: Task }) => (
    <View style={[styles.taskCard, { backgroundColor: colors.surface }, shadows.md]}>
      <View style={styles.taskHeader}>
        <Text style={[styles.taskTitle, { color: colors.text }]} numberOfLines={1}>
          {item.title}
        </Text>
        {(isAdmin || (item.assignedTo.includes(user?.uid || '') && user?.role === 'developer')) && (
          <View style={styles.taskActions}>
            <TouchableOpacity onPress={() => openModal(item)} style={[styles.actionButton, { backgroundColor: colors.primary + '10' }]}>
              <Icon name="edit" size="sm" tintColor={colors.primary} />
            </TouchableOpacity>
            {isAdmin && (
              <TouchableOpacity onPress={() => handleDelete(item)} style={[styles.actionButton, { backgroundColor: colors.error + '10' }]}>
      ...
      {isAdmin && item.comments && item.comments.length > 0 && (
        <View style={styles.taskCommentsSection}>
          ...
          >
            <Text style={[styles.commentsToggleText, { color: colors.primary }]}>
          ...
          {expandedComments[item.id] && (
            <View style={[styles.taskCommentsDropdown, { backgroundColor: colors.background }]}>
              {item.comments.slice(-3).map((comment) => (
                <View key={comment.id} style={styles.taskCommentItem}>
                  <View style={styles.taskCommentHeader}>
                    <Text style={[styles.taskCommentAuthor, { color: colors.primary }]}>
                      {comment.userName}
                    </Text>
                    <Text style={[styles.taskCommentTime, { color: colors.textSecondary }]}>
                      {new Date(comment.timestamp).toLocaleDateString('en-US', { month: 'short', day: 'numeric' })}
                    </Text>
                  </View>
                  <Text style={[styles.taskCommentText, { color: colors.text }]} numberOfLines={2}>
                    {comment.text}
                  </Text>
                </View>
              ))}
              {item.comments.length > 3 && (
                <TouchableOpacity onPress={() => openModal(item)} style={styles.viewAllComments}>
        ...
        <View style={[styles.modalContainer, { backgroundColor: colors.background }]}>
          ...
          <ScrollView style={styles.modalContent}>
            ...
            {!isAdmin && selectedTask && (
              <View style={styles.inputGroup}>
                <Text style={[styles.inputLabel, { color: colors.text }]}>Task: {selectedTask.title}</Text>
            ...
            {isAdmin && (
              <View style={styles.assigneeSection}>
                <View style={styles.assigneeHeader}>
                  <Text style={[styles.inputLabel, styles.assigneeLabelFlex, { color: colors.text }]}>Assignee *</Text>
                  <TouchableOpacity
                    onPress={() => {
                ...
                <Dropdown
                  data={usersLoading ? [
                    ...
                    label: approvedUser.name || approvedUser.displayName || approvedUser.email?.split('@')[0] || 'Unknown User',
                  ...
                  placeholder={usersLoading ? "Loading users..." : approvedUsers.length === 0 ? "No users available" : "Select a user to assign"}
            ...
            <View style={styles.inputGroup}>
              <Text style={[styles.inputLabel, { color: colors.text }]}>Today's Date</Text>
              <View style={[styles.todayDateContainer, { backgroundColor: colors.surface }]}>
                ...
                <Text style={[styles.todayDateLabel, { color: colors.textSecondary }]}>
            ...
            <View style={styles.row}>
              {isAdmin && (
                <View style={[styles.inputGroup, styles.priorityInputContainer]}>
                  <Dropdown
                    label="Priority"
                    data={[
                      { label: 'Low', value: 'Low' },
              ...
              <View style={[styles.inputGroup, isAdmin ? styles.statusInputContainer : styles.statusInputContainerNoMargin]}>
                <Dropdown
                  label="Status"
                  data={[
                  ...
                  onSelect={(value) => setTaskForm({ ...taskForm, status: value as any })}
              ...
              <View style={styles.row}>
                <View style={[styles.inputGroup, styles.dateInputContainer]}>
                  ...
                  <TextInput
                    ...
                    onChangeText={(text) => setTaskForm({ ...taskForm, startDate: text })}
                    style={[styles.textInput, { backgroundColor: colors.surface, color: colors.text }]}
                ...
                <View style={[styles.inputGroup, styles.dateEndInputContainer]}>
                  ...
                  <TextInput
                    ...
                    onChangeText={(text) => setTaskForm({ ...taskForm, endDate: text })}
                    style={[styles.textInput, { backgroundColor: colors.surface, color: colors.text }]}
                    placeholder="YYYY-MM-DD"
                    placeholderTextColor={colors.textSecondary}
                  />
            ...
            {selectedTask && selectedTask.comments && selectedTask.comments.length > 0 && (
              <View style={styles.inputGroup}>
                <Text style={[styles.inputLabel, { color: colors.text }]}>Comments ({selectedTask.comments.length})</Text>
            ...
            {(selectedTask && !isAdmin) && (
              <View style={styles.inputGroup}>
                <Text style={[styles.inputLabel, { color: colors.text }]}>Add Comment</Text>
                <TextInput
                  value={taskForm.comment}
                  onChangeText={(text) => setTaskForm({ ...taskForm, comment: text })}
                  style={[styles.textArea, { backgroundColor: colors.surface, color: colors.text }]}
            ...
            {(selectedTask && isAdmin) && (
              <View style={styles.inputGroup}>
                <Text style={[styles.inputLabel, { color: colors.text }]}>Add Comment (Optional)</Text>
                <TextInput
                  value={taskForm.comment}
                  onChangeText={(text) => setTaskForm({ ...taskForm, comment: text })}
                  style={[styles.textArea, { backgroundColor: colors.surface, color: colors.text }]}
